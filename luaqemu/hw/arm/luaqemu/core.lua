...
typedef int     (*TprReadCb)(TprReadCbArgs *args);
...
function lua_load_file(path,addr)
    C.lua_load_file(path,addr)
end
...
function lua_get_cp15_register(reg_name)
    return ffi.cast('uint64_t', C.lua_get_cp15_register(reg_name))
end
...
-- courtesy of Michal Kottmanjianshen
...
-- add a new memory region for peripheral
function add_peripheral_memory_region(addr,size)
    C.add_peripheral_memory_region(addr,size)
end
