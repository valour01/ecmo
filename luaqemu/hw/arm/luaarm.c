...
static void util_watchpoint_insert(uint64_t addr, uint64_t size, int flags, watchpoint_cb cb)
{
    ...
    wp->flags = flags;
    wp->fptr = cb;
    debug_print("before insert the watchpoint\n");

    cpu_watchpoint_insert(luastate.cs, addr, size, flags, NULL);
    debug_print("after insert the watchpoint\n");
...
static void util_watchpoint_remove(uint64_t addr, uint64_t size, int flags)
{
    ...
    for(iterator = watchpoints; iterator; iterator = iterator->next) {
        wp = iterator->data;
        debug_print("find watchpoint covering '%" PRIx64 "-%" PRIx64 " (flags: %x)\n", wp->addr, wp->len, wp->flags);
...
{
    debug_print("adding breakpoint for '%" PRIx64 "' -> '%d'\n", addr, bp_func);
...
static void util_breakpoint_remove(uint64_t addr)
{
    ...
    debug_print("removing breakpoint for '%" PRIx64 "'\n", addr);
...
uint64_t trapped_physregion_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    cbArgs.addr = addr2;
    cbArgs.size = size;
    uint32_t value = tpr->readCb(&cbArgs);
...
static void util_trapped_physregion_add(uint64_t addr, uint64_t size, TprReadCb readCb, TprWriteCb writeCb)
{
    ...
    debug_print("adding trapped physregion '%" PRIx64 "-%" PRIx64 "'\n", addr, (addr + size));    
...
static void util_trapped_physregion_remove(uint64_t addr, uint64_t size)
{
    ...
    for(iterator = trapped_physregions; iterator; iterator = iterator->next) {
        ...
        if (tpr->region.addr == addr && tpr->region.size == size) {
            debug_print("removing trapped physregion '%" PRIx64 "-%" PRIx64 "'\n", addr, (addr + size));
    ...
    debug_print("could not remove trapped physregion '%" PRIx64 "-%" PRIx64 "'\n", addr, (addr + size));
}
...
static void init_memory_regions(void)
{
    ...
    while (lua_next(lua_state, -2)) {
        debug_print("adding memory region '%s'\n", lua_tostring(lua_state, -2));
        /*debug_print("%s - %s\n",
...
/* Board */
static struct arm_boot_info arm_binfo;

static void load_arm_elf(const char *file_path)
{
    ARMCPU *cpu = ARM_CPU(luastate.cs);

    arm_binfo.ram_size = 0x500000;
    arm_binfo.kernel_filename = file_path;
    arm_load_kernel(cpu, &arm_binfo);
    ...
    //     exit(7);
    // }
    // debug_print("ELF entry is at: 0x%lu\n", elf_entry);
}
...
static void add_file(void)
{
    ...
    if (mapping_type && !strcasecmp(mapping_type, "elf")) {
        ...
        binfo.board_id = lua_get_unsigned("board_id",THROW_ERROR);
        //mapping_start = lua_get_unsigned("start", THROW_ERROR);
        //mapping_size  = lua_get_unsigned("size", THROW_ERROR);
        //debug_print("trying to flat load file '%s' to 0x%" PRIx64 " (0x%" PRIx64 ")\n", mapping_fn, mapping_start, mapping_size);
        //load_flat_file(mapping_fn, mapping_start, mapping_size);
        binfo.ram_size = lua_get_unsigned("ram_size",THROW_ERROR);
        binfo.kernel_filename = luastate.machine->kernel_filename;
        debug_print("The kernel filename is: %s\n",binfo.kernel_filename);
        binfo.kernel_cmdline = luastate.machine->kernel_cmdline;
        binfo.initrd_filename = luastate.machine->initrd_filename;
        debug_print("before load kernel\n");
        arm_load_kernel(luastate.cpu,&binfo);
        debug_print("after load kernel\n");
    }
}
...
static void init_file_mappings(void)
{
    ...
    lua_pushnil(lua_state);
    while (lua_next(lua_state, -2)) {
        debug_print("adding file mapping '%s'\n", lua_tostring(lua_state, -2));
        /*debug_print("%s - %s\n",
              lua_typename(lua_state, lua_type(lua_state, -2)),
              lua_typename(lua_state, lua_type(lua_state, -1))); */
...
static void init_reset_addr(int type)
{
    ...
    debug_print("reset address is: 0x%" PRIx64 "\n", addr);
...
void lua_cpu_do_interrupt_callback(const CPUState *cpu)
{
    if (luastate.do_interrupt_cb == 0) return;
    
    lua_rawgeti(lua_state, LUA_REGISTRYINDEX, luastate.do_interrupt_cb);
    lua_pushinteger(lua_state, (lua_Integer)cpu->exception_index);
    lua_stackdump(lua_state);
    if (lua_pcall(lua_state, 1, 0, 0)) {
        error_report("failed to call do_interrupt_cb (%d): %s\n", luastate.do_interrupt_cb, lua_tostring(lua_state, -1));
    }
}

void lua_cpu_post_do_interrupt_callback(const CPUState *cpu)
{
    if (luastate.post_do_interrupt_cb == 0) return;
    
    lua_rawgeti(lua_state, LUA_REGISTRYINDEX, luastate.post_do_interrupt_cb);
    lua_stackdump(lua_state);
    if (lua_pcall(lua_state, 0, 0, 0)) {
        error_report("failed to call post_do_interrupt_cb (%d): %s\n", luastate.post_do_interrupt_cb, lua_tostring(lua_state, -1));
    }
}
...
{
    debug_print("Found stuck state callback. Make sure to set \"stuck_max\" in env block %s.\n", "");
...
static void init_cpu_env(int type)
{
    ...
    /* yajin: init trustzone */
    //luastate.cpu->has_el3 = true;

    periph_base = lua_get_unsigned("periph_base", 0);
    if (periph_base != 0) {
        debug_print("setting peripheral base for cbar to %" PRIx64 "\n", periph_base);
    ...
    if (luastate.cs->crs.miss_max) {
        debug_print("waiting for max %" PRIu64 " cycles\n", luastate.cs->crs.miss_max);
    }
...
static const cb_keyword_table_t cb_kwt[] =
{
    ...
    {"do_interrupt_cb", &luastate.do_interrupt_cb, NULL},
    {"post_do_interrupt_cb", &luastate.post_do_interrupt_cb, NULL},
    {{0, 0, 0}}
};
...
static void init_cpu_callbacks(int type)
{
    ...
    for (;i < n; i++) {
        ...
        *(cb_kwt[i].lref) = luaL_ref(lua_state, LUA_REGISTRYINDEX);
        debug_print("found callback for %s (%d)\n", cb_kwt[i].keyword, *(cb_kwt[i].lref));
...
{
   debug_print("in the function init_cpu_state\n");
    int m_type = 0;
    ...
    debug_print("finish the function init_cpu_state\n");
...
extern uint64_t lua_get_cp15_register(const char *reg_name);
...
extern void lua_init_ic_timer_uart(hwaddr base,int it_shift, int irq_num);
extern void lua_load_file(const char* path, uint64_t addr);

void lua_init_ic_timer_uart(hwaddr base,int it_shift,int irq_num){
    debug_print("Init IC\n");
    int n;
    DeviceState *dev;
    qemu_irq pic[32];
    dev = sysbus_create_varargs("pl190", 0x10140000,
                                qdev_get_gpio_in(DEVICE(luastate.cpu), ARM_CPU_IRQ),
                                qdev_get_gpio_in(DEVICE(luastate.cpu), ARM_CPU_FIQ),
                                NULL);

    for (n = 0; n < 32; n++) {
        pic[n] = qdev_get_gpio_in(dev, n);
    }
    debug_print("Init Timer\n");
    sysbus_create_simple("sp804", 0x101e0000, pic[3]);
    sysbus_create_simple("sp804", 0x101e2000, pic[4]);
    sysbus_create_simple("sp804", 0x101e3000, pic[5]);
    debug_print("Init UART\n");
    //if (type == 0){
    if (true){	    
    serial_mm_init(get_system_memory(), base, it_shift, pic[irq_num], 115200, serial_hd(0), DEVICE_LITTLE_ENDIAN);}
    //if (type == 1){
    if (false){
	Error *error_abort = NULL;
	object_initialize(&luastate.imx_uart,sizeof(luastate.imx_uart),TYPE_IMX_SERIAL);
	qdev_set_parent_bus(DEVICE(&luastate.imx_uart),sysbus_get_default());

	qdev_prop_set_chr(DEVICE(&luastate.imx_uart),"chardev",serial_hd(0));
	object_property_set_bool(OBJECT(&luastate.imx_uart), true, "realized", &error_abort);
	
	if (error_abort){
		debug_print("realize error");
	}
	sysbus_mmio_map(SYS_BUS_DEVICE(&luastate.imx_uart),0,base);
	sysbus_connect_irq(SYS_BUS_DEVICE(&luastate.imx_uart),0,pic[irq_num]);
    }
    debug_print("Finished init UART\n");
...
void lua_load_file(const char* path, uint64_t addr){
    int fd;
    ssize_t did;
    debug_print("Load the file %s at 0x%x\n",path,addr);
    fd = open(path,O_RDONLY | O_BINARY);
    int64_t f_size = get_image_size(path);
    debug_print("Alloc the image size 0x%x\n",f_size); 
    uint8_t *buf = g_malloc(f_size);
    did = read(fd,buf,f_size);
    cpu_physical_memory_write(addr,buf,f_size);
    close(fd);
}
...
uint64_t lua_get_cp15_register(const char *reg_name)
{
    if (reg_name != NULL) {
        if ((strncmp(reg_name, "mvbar", 5) == 0) && (strlen(reg_name) == 5)) {
            return luastate.cpu->env.cp15.mvbar;
        } else if ((strncmp(reg_name, "vbar_s", 6) == 0) && (strlen(reg_name) == 6)) {
            return luastate.cpu->env.cp15.vbar_s;
        } else if ((strncmp(reg_name, "vbar_ns", 7) == 0) && (strlen(reg_name) == 7)) {
            return luastate.cpu->env.cp15.vbar_ns;
        }
    }
...
static void lua_vm_state_change(void *opaque, int running, RunState state)
{
    ...
    */
    ...
    //debug_print("VM state change at: %" PRIx64 ", state %d \n", old_pc, state);
...
static void init_vm_states(void)
{
    ...
    debug_print("added %d breakpoints...\n", g_hash_table_size(breakpoints));
}
...
static void init_luastate(MachineState *machine)
{
    ...
    cpu_oc = cpu_class_by_name(TYPE_ARM_CPU, machine->cpu_type);
    if (!cpu_oc) {
...
static void lua_init(MachineState *machine)
{
    ...
    if (NULL == machine->cpu_type) {
        lua_get_global("machine_cpu", THROW_ERROR);
        machine->cpu_type = lua_tostring(lua_state, -1);
        debug_print("found machine cpu type: %s\n", machine->cpu_type);
    }
    ...
    if (lua_isnil(lua_state, -1)) {
        debug_print("no post_init function to be called\n");
    }

    if (lua_pcall(lua_state, 0, 0, 0)) {
        debug_print("problem during post_init function call: %s\n", lua_tostring(lua_state, -1));
    }
}
